...
cor.plt=na.omit(cor.plt)  #Get rid of the junk we flagged above
cor.plt=subset(cor.plt, abs(cor.plt$Freq)>0.8)
...
We randomly partitioned our dataset into training and validation subsets with 70% for training and 30% set aside for testing.
...
linreg3 <- caret::train(PH~Mnf.Flow+Usage.cont+Alch.Rel+Bowl.Setpoint+Pressure.Vacuum+Temperature+Oxygen.Filler+Hyd.Pressure3+Carb.Pressure1+Hyd.Pressure1, data=traindata, method="lm",  trControl=trctrl)
...
linreg4 <- caret::train(PH~Mnf.Flow+Usage.cont+Alch.Rel+Bowl.Setpoint+Temperature+Oxygen.Filler+Hyd.Pressure3+Carb.Pressure1, data=traindata, method="lm", trControl=trctrl)
...
postResample(pred = forPredA, obs = testdataA$PH)
...
postResample(pred = forPredB, obs = testdataB$PH)
...
postResample(pred = forPredC, obs = testdataC$PH)
...
postResample(pred = forPredD, obs = testdataD$PH)
```
