...
postResample(pred = linPred, obs = testdata$PH) #0.1414880 0.3775156

##Ridge Regression
ridge <- caret::train(PH~., data=traindata, method="ridge", 
                trControl=trctrl, preProcess=c("center","scale"))
ridgePred <- predict(ridge, newdata = testdata)
postResample(pred = ridgePred, obs = testdata$PH) #0.1414837 0.3775762

##Lasso Regression
lasso <- caret::train(PH~., data=traindata, method="lasso", 
                trControl=trctrl, preProcess=c("center","scale"))
lassoPred <- predict(lasso, newdata = testdata)
postResample(pred = lassoPred, obs = testdata$PH) #0.1418941 0.3762947
...
postResample(pred = forPred, obs = testdata$PH) #0.11550844 0.59451507
...
postResample(pred = bagPred, obs = testdata$PH) #0.12790436 0.50718903
...
postResample(pred = ctrePred, obs = testdata$PH) #0.1519582 0.2804008
...
postResample(pred = cartPred, obs = testdata$PH) #0.1593219 0.2053639

##MARS
marsFit <- earth(PH~., data = traindata, degree=2, nprune=14)
marsPred <- predict(marsFit, newdata = testdata)
postResample(pred = marsPred, obs = testdata$PH) #0.1399919 0.3912855
...
postResample(pred = knnPred, obs = testdata$PH) #0.1278504 0.5088682
...
postResample(pred = svmPred, obs = testdata$PH) #0.13136218 0.48751241
```
